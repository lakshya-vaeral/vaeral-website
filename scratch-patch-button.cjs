...
// Safely inject without mangling original HTML
...
if (grid.length) {
    ...
    if (indexHtml.includes(gridHtml)) {
        ...
        console.log("Successfully replaced exact grid HTML block.");
    } else {
        console.log("$.html(grid) didn't exactly match original HTML. Falling back to cheerio modification.");
        grid.after(buttonHtml);
        indexHtml = $.html();
    }
} else {
...
console.log(`New size: ${indexHtml.length}`);
console.log(`Difference: ${indexHtml.length - originalLength}`);
fs.writeFileSync('dist/test-index.html', indexHtml);
