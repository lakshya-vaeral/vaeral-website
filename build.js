...
function main() {
  ...
  if (fs.existsSync(indexFile)) {
    ...
    const jsFix = `\\n<script>\\n  setInterval(() => {\\n    document.querySelectorAll('nav a[href="#contact"]').forEach(contactLink => {\\n      // Find the top-level container of the link within the nav to clone\\n      let container = contactLink;\\n      while (container.parentElement && container.parentElement.tagName.toLowerCase() !== 'nav') {\\n        if (container.parentElement.style.display === 'contents' || container.parentElement.style.display === 'flex') break;\\n        container = container.parentElement;\\n      }\\n      \\n      if (!container.parentElement.querySelector('.nav-blog-injected')) {\\n        const blogNode = container.cloneNode(true);\\n        blogNode.classList.add('nav-blog-injected');\\n        \\n        // Update link and text\\n        const a = blogNode.tagName === 'A' ? blogNode : blogNode.querySelector('a');\\n        if (a) {\\n          a.href = '/blog';\\n          a.target = '_top';\\n        }\\n        \\n        const textNode = blogNode.querySelector('p, span');\\n        if (textNode) textNode.textContent = 'Blogs';\\n        \\n        // Insert before Contact\\n        container.parentNode.insertBefore(blogNode, container);\\n      }\\n    });\\n  }, 500);\\n</script>\\n`;
