...
/* The class is added by the observer below, at the point in the scroll where Framer's own
   appear animation fires on the other two cards. */
.framer-ofgoy .framer-6w8gru.vaeral-ps-swept .framer-153kduw [data-framer-background-image-wrapper]::after {
  animation: vaeral-ps-sweep 1.28s cubic-bezier(0.2, 0, 0.35, 1) forwards;
}
@keyframes vaeral-ps-sweep {
...
(function () {
  function arm() {
    ...
    if (!card || card.classList.contains('vaeral-ps-swept')) return;
    if (!('IntersectionObserver' in window)) { card.classList.add('vaeral-ps-swept'); return; }
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (!e.isIntersecting) return;
        e.target.classList.add('vaeral-ps-swept');
        io.disconnect();
      });
    }, { threshold: 0.25 });
    io.observe(card);
