...
(function () {
  var done = false;
  // 1.6s on a symmetric ease-in-out, both measured off the Reddit card's bar frame by frame.
  // Opacity tracks the eased position exactly rather than running on a curve of its own.
  function sweep() {
    if (done) return;
    done = true;
    window.removeEventListener('scroll', check);
    var root = document.documentElement, t0 = null;
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) {
      root.style.setProperty('--vaeral-ps-y', '0px');
      root.style.setProperty('--vaeral-ps-o', '1');
      return;
    }
    function step(ts) {
      if (t0 === null) t0 = ts;
      ...
      root.style.setProperty('--vaeral-ps-y', (-150 * (1 - e)).toFixed(2) + 'px');
      root.style.setProperty('--vaeral-ps-o', e.toFixed(4));
      if (x < 1) requestAnimationFrame(step);
    }
    requestAnimationFrame(step);
  }
  // Deliberately not an IntersectionObserver. Its callback is queued behind whatever else the
  // main thread is doing, and Framer's appear animations keep it busy exactly as this section
  // arrives, which started the sweep ~380ms after the other two bars had left.
  //
  // The check runs in the scroll event, which is the same task as the scroll itself, so the
  // sweep begins on the same frame Framer starts its two. A rAF loop backs it up for the case
  // where the card is already in view without a scroll ever firing. One rect read per frame,
  // stopping the moment it starts; rAF is suspended while the tab is hidden.
  function check() {
    if (done) return true;
    var card = document.querySelector('.framer-6w8gru');
    if (!card) return false;
    if (card.getBoundingClientRect().top < window.innerHeight) { sweep(); return true; }
    return false;
  }
  function arm() {
    if (!check()) requestAnimationFrame(arm);
  }
  // Framer rebuilds this subtree on hydration, so keep re-arming for a while after load.
  function start() {
    // Fetch both halves up front. A CSS animation on a pseudo-element whose background is still
    // loading stays PENDING (startTime null) and only begins once the image lands, which put the
    // sweep ~650ms behind the other two cards. The section is far down the page, so warming the
    // cache here costs nothing by the time anyone scrolls to it.
    ...
    setTimeout(function () { mo.disconnect(); arm(); }, 8000);
    // The properties live on :root, so a Framer re-render cannot undo a finished sweep.
  }
